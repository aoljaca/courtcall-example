            ...
            <v-container fluid>
                <v-row>
                    <v-col>
                        ...
                        return-object
                        :label="$t('meetingUI.controlBar.more.inviteParticipants.selectType')">
                        </v-select>
                    </v-col>
                </v-row>
                <v-row v-if="shouldShowNameEntry()">
                    <v-col>
                        <v-text-field :label="$t('meetingUI.controlBar.more.inviteParticipants.enterName')"></v-text-field>
                    </v-col>
                </v-row>
                <v-row v-if="shouldShowCaseSelection()">
                    <v-col>
                        ...
                        return-object
                        :label="$t('meetingUI.controlBar.more.inviteParticipants.selectCase')"></v-select>
                    </v-col>
                </v-row>
                <v-row v-if="link">
                    <v-col>
                    ...
                    <v-col cols="2" class="d-flex justify-end">
                        <v-btn icon>
                            <v-icon @click="copyLink()" :title="$t('general.copy')">mdi-content-copy</v-icon>
                        </v-btn>
                    </v-col>
                </v-row>
                <v-row>
                    ...
                    <v-col class="d-flex justify-end">
                        <v-btn @click="generateLink()" elevation="0">{{$t('meetingUI.controlBar.more.inviteParticipants.generateLink')}}</v-btn>
                    </v-col>
                </v-row>
            </v-container>
        </v-card>
...
import {InviteParticipantType, INVITE_PARTICIPANT_TYPES} from '@/model/meeting/meeting-ui/control-bar/more/invite-participants/invite-participant-type';
