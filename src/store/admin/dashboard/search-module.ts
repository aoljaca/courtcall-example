...
const searchModule: Module<any, any> = {
  ...
  actions: {
    async search(
      { commit, rootGetters },
      query: { type: SearchType; searchTerm: string }
    ) {
      ...
      switch (query.type) {
        case SearchType.SYSTEM_USER:
          ...
          response = systemUsers.filter((su) =>
            su.name.toLowerCase().includes(query.searchTerm)
          );
          break;
        case SearchType.PARTICIPANT:
          participants = rootGetters["ParticipantsModule/getAsList"];
          response = participants.filter((p) =>
            p.name.toLowerCase().includes(query.searchTerm)
          );
          break;
        case SearchType.ROOM:
          rooms = rootGetters["RoomModule/getAsList"];
          response = rooms.filter((r) =>
            r.roomDetails.name?.toLowerCase().includes(query.searchTerm)
          );
          break;
        case SearchType.CASE:
          cases = rootGetters["CasesModule/getAsList"];
          response = cases.filter((c) =>
            c.name.toLowerCase().includes(query.searchTerm)
          );
          break;
        case SearchType.ORGANIZATION:
          organizations = rootGetters["OrganizationsModule/getAsList"];
          response = organizations.filter((o) =>
            o.name.toLowerCase().includes(query.searchTerm)
          );
          break;
        default:
          break;
  ...
  getters: {
    ...
    searchTypeByName: (state, getters) => (name: string) => {
      return getters.searchTypes.find(
        (st: LocalizedSearchType) => st.name === name
      );
    },
  },
};
