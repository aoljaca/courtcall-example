...
export class BackgroundBlurServiceImpl implements IBackgroundBlurService {
    ...
    alterVideo(videoElementId: string, canvasElementId: string, backgroundOptions: BackgroundOption, store: Store<any>) {
        ...
        store.dispatch('BackgroundBlurModule/changeModeAction',{mode: 'loading'});
        ...
        store.dispatch('BackgroundBlurModule/updateTimeStampAction',{timestamp: dateTime});
    ...
    private async segmentBackgroundInRealTime(net: BodyPix.BodyPix, options: BindPageParams) {
    ...
    private async segmentBlurInRealTime(net: BodyPix.BodyPix, options: BindPageParams) {
        const segmentation = await net.segmentPerson(options.videoElement);
        const context = options.canvasElement.getContext('2d')!;
        BodyPix.drawBokehEffect(document.getElementById('canvas-preview') as HTMLCanvasElement, options.videoElement, segmentation, 6, 2, false);
